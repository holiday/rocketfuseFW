 ...
 * @method     AdminQuery orderByUsername($order = Criteria::ASC) Order by the username column
 ...
 * @method     AdminQuery groupByUsername() Group by the username column
 ...
 * @method     Admin findOneByUsername(string $username) Return the first Admin filtered by the username column
 ...
 * @method     array findByUsername(string $username) Return Admin objects filtered by the username column
...
abstract class BaseAdminQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'ready4car', $modelName = 'Admin', $modelAlias = null)
	{
	...
	{
		$sql = 'SELECT `ID`, `USERNAME`, `PASSWORD` FROM `Admins` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Admin();
			$obj->hydrate($row);
			AdminPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	...
	public function filterByUsername($username = null, $comparison = null)
	{
		...
		return $this->addUsingAlias(AdminPeer::USERNAME, $username, $comparison);
	}
